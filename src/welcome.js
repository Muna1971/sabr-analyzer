...
const GOOGLE_FORM_CONFIG = {
    ...
    formActionUrl: 'https://docs.google.com/forms/d/e/1FAIpQLSdoKQIRv1Oy-GsX23oa5s3rTXYVodqZyYTsp2YQqZ5mPjl9bQ/formResponse',
    // Each Google Form field has an entry ID like 'entry.123456789'
    fields: {
        name: 'entry.768876331',
        email: 'entry.85704027',
        organization: 'entry.1623226452',
        timestamp: 'entry.748290318',
    },
};
...
async function handleSubmit(e) {
    ...
    if (!name) {
        ...
        toast('Ø§ÙØ±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù Ø§ÙØ§Ø³Ù', 'error');
        return;
    }
    if (!isValidEmail(email)) {
        emailEl?.classList.add('error');
        toast('Ø§ÙØ±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù Ø¨Ø±ÙØ¯ Ø¥ÙÙØªØ±ÙÙÙ ØµØ­ÙØ­', 'error');
        return;
    }
    ...
    // Save locally regardless of network outcome â we still grant founding member
    ...
    refreshBadge();
    hideWelcomeModal();
    toast('ð ÙØ±Ø­Ø¨Ø§Ù Ø¨Ù ÙØ§ Ø¹Ø¶Ù ÙØ¤Ø³Ø³!', 'success', 4500);
}
...
function toast(msg, type = 'info', duration = 3000) {
    ...
    const icons = { success: 'â', error: 'â', info: 'â¹', warning: 'â ' };
...
async function submitToGoogleForm(data) {
    ...
    if (!cfg.formActionUrl || cfg.formActionUrl.indexOf('docs.google.com') === -1) {
        // Form not configured yet â store locally and exit silently.
